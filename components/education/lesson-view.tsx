...
import { XP_PER_CORRECT } from "@/lib/education";
import type { Lesson, LessonBlock } from "@/lib/courses";
import { maxLessonXp } from "@/lib/courses";
import { Quiz, type QuizResult } from "@/components/education/quiz";
import {
  useProgress,
  type CompleteResult,
} from "@/components/education/progress-provider";
...
function Block({ block }: { block: LessonBlock }) {
  switch (block.type) {
    ...
    case "paragraph":
      return <p className="text-base/8 text-muted-foreground">{block.text}</p>;
    case "list":
      return (
    ...
    case "callout": {
      ...
      return (
        <div className={cn("flex gap-3 rounded-xl border p-4", config.className)}>
          ...
          <p className="text-sm/6 text-foreground">{block.text}</p>
        </div>
      );
    }
  }
